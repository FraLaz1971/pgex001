...
C GRC: Get Random Character
      CHARACTER*1 FUNCTION GRC(I)
	    ...
	    GRC=CHAR(INT(R*256))
      END
C WBA: Write Binary Array (with random 8bits data)
...
9999    RETURN
      END
C WAA: Write Ascii Array
C WIDTH: x-axis dimension HEIGHT: y-axis dimension OFNAM: output file name
...
C AND SAVE TO BINARY FILE
      SUBROUTINE RAASB(IFNAM)
        IMPLICIT NONE
...
C        INTEGER ROW(MXIDIM)
        ...
        INTEGER I,J,WIDTH,HEIGHT
        DEBUG = .FALSE.
C Create output file name, same basename, .raw suffix
        POSI=INDEX(IFNAM,'.')
